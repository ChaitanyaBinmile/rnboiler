...
const CustomForm: React.FC<CustomFormProps> = ({ fields, onSubmit, buttonLabel = 'Submit', validationSchema,children}) => {
  ...
  const [currentFieldOnChange, setCurrentFieldOnChange] = useState<((value: any) => void) | null>(null); 
  const [currentFieldType, setCurrentFieldType] = useState<'date' | 'time' | null>(null);
  ...
  const getLabelStyle = (fieldName: string ) => {
   ...
   const handleDateConfirm = () => {
    ...
    if (currentFieldOnChange && date) {
      const formattedValue = currentFieldType === 'date'
              ...
              <Controller
                ...
                render={({ field: { onChange, onBlur, value } }) => (
                  ...
                  <TextInput
                    ...
                    value={field.name === 'date' || field.name === 'time' ? (value ? value.toString() : '') : value.toString()}
